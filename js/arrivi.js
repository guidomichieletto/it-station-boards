var updateTrains = window.setInterval(function(){
  $.get("api/LAP-token/stazione/arrivi/treni/2", function(data,status){
    var rows = "";
    ...
    data.forEach(function(dt){
      ...
      trow += "<td>" + dt.Provenienza + "</td><td>" + dt.Orario + "</td><td>" + ritardo + "</td><td>" + dt.Binario + "</td><td>" + partenza + "</td></tr>";
...
var annunci = window.setInterval(function(){
  $.get("api/LAP-token/stazione/arrivi/annunci/2", function(data,status){
    var data = JSON.parse(JSON.stringify(data));
